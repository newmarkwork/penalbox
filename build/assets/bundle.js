  ...
  .replace(/ /g, '.')}`;
	...
	if (sliders.length) {
	  sliders.forEach(slider => {
	    const pagination = slider.querySelector('.product-card-inner-slider-pagination');
	...
	if (map) {
	  ...
	  function init() {
	    ...
	    // Метка
	    const officePlacemark = new ymaps.Placemark(JSON.parse('[' + map.dataset.coords + ']'), {
	      iconContent: map.dataset.iconContent ? `<div class="ymaps-icon-content-layout-inner">${map.dataset.iconContent}</div>` : ''
	    }, {
	      // Опции.
	...
	if (nav && navOpener && navCloser) {
	  ...
	  navOpener.addEventListener('click', onClickOpenNav);
	  const mobileNavListExpanders = nav.querySelectorAll('.main-nav__list-item-expand-btn');
